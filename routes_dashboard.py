...
def get_next_available_ips(parent_cidr: str, db: Session, block_id: int, limit=100):
    ...
    used_networks = [ipaddress.ip_network(s.cidr) for s in used_subnets]

    # Get all single IP assignments in the block
    ...
    for addr in all_interface_addresses:
        try:
            ...
            if ip in parent_net:
                # Treat each single IP as a /32 network for exclusion
                used_networks.append(ipaddress.ip_network(f"{addr.ip}/32"))
        except ValueError:
            continue

    # Collapse and sort the used networks
    if not used_networks:
        # If no usage, the first IP is the start of the network
        return [str(ip) for ip in parent_net.hosts()][:limit]

    # This function is not perfect, a more robust solution would use `netaddr` library
    # or a more complex algorithm to handle all edge cases of collapsing and summarizing.
    # For now, we sort and find gaps.
    used_networks.sort()
    ...
    for used_net in used_networks:
        if used_net.network_address > last_ip:
            gap_start = last_ip + 1
        ...
        # Move last_ip to the end of the current used network
        if used_net.broadcast_address > last_ip:
...
def manual_allocate_action(
    ...
    if mask == 32:
        # --- Handle as Single IP Assignment ---
        try:
            ip_addr = ipaddress.ip_address(starting_ip)

            # 1. Check for overlap with existing subnets
            all_subnets = db.query(models.Subnet).all()
            for s in all_subnets:
                if ip_addr in ipaddress.ip_network(s.cidr):
                     return render_form_with_error(f"IP address {starting_ip} is part of an existing subnet ({s.cidr}). Use the 'Devices' page to assign IPs from existing subnets.")

            # 2. Check for existing single IP
            existing_ip = db.query(models.InterfaceAddress).filter(models.InterfaceAddress.ip == starting_ip).first()
            if existing_ip:
                return render_form_with_error(f"IP address {starting_ip} is already assigned.")
